...
def login(request):
    if request.method == "POST":
        ...
        try:
            bde_user = BDE_User.objects.get(username=username)
            if bde_user.password == password:
                request.session['user'] = username
                return redirect('dashboard')
                # return render(request,'dashboard.html',{'bde_user':bde_user})
            else:
                return render(request, 'login.html', {'errors': 'Invalid password.'})
        except BDE_User.DoesNotExist:
...
def project(request):
    if request.method == "POST":
        try:
            ...
            resume_shared = request.POST.get('resume_shared', 'NO')  # Default value is 'NO'
...
def round1(request, project_id):
    if request.method == "POST":
        ...
        return render(request, 'round1.html', {'project_id': project_id})
def round2(request, project_id):
    if request.method == "POST":
        ...
        return render(request, 'round2.html', {'project_id': project_id})
...
def round3(request, project_id):
    if request.method == "POST":
        ...
        return render(request, 'round3.html', {'project_id': project_id})
